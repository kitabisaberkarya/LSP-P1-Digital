...
const Footer: React.FC<FooterProps> = ({ language }) => {
  ...
  return (
    <footer className="bg-slate-800 dark:bg-slate-900 text-slate-300 border-t border-slate-700 dark:border-slate-800">
      <div className="container mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div>
            <h3 className="text-lg font-semibold text-white">LSP P1 SMK DR. SOETOMO</h3>
            <p className="mt-2 text-slate-400">
              {t.about[language]}
            </p>
          </div>
          <div>
            <h3 className="text-lg font-semibold text-white">{t.contact[language]}</h3>
            <ul className="mt-2 space-y-2 text-slate-400">
              <li className="flex items-center">
                ...
                <span>lspsmkdr.soetomosby@gmail.com</span>
              </li>
              <li className="flex items-center">
                ...
                <span>031 - 5914480</span>
              </li>
            </ul>
          </div>
          <div>
            <h3 className="text-lg font-semibold text-white">{t.follow[language]}</h3>
        ...
        <div className="mt-8 border-t border-slate-700 pt-8 text-center text-slate-400 text-sm">
          <p>&copy; {new Date().getFullYear()} LSP P1 SMK DR. SOETOMO SURABAYA. {t.copyright[language]}</p>
          <p className="mt-1">{t.developedBy[language]} <a href="#" className="font-semibold text-slate-300 hover:text-white">Ari Wijaya</a>.</p>
        </div>
      </div>
    </footer>
  );
